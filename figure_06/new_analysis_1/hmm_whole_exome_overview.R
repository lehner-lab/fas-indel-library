...
for (i in 1:length(my_exons)) {
  ...
  spliceAI_predictions_file <- paste("../fas-indel-library/figure_06/hmm/data/exon_predictions/",
...
save(tibble_exome_overview,
     file = "hmm_tibble_exome_overview.RData")

ggplot(data = tibble_exome_overview,
       mapping = aes(x = log2(length),
                     y = sum_S + sum_E)) +
  geom_point() +
  geom_smooth() +
  theme_bw() +
  ggtitle("ASD") +
  scale_y_continuous(breaks = seq(1,10000,1000))
