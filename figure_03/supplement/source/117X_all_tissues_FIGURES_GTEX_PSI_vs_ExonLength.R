...
for (each_tissue_name in unique(Plot.DF$Tissue)) {
  ...
  proportions_matrix <- sapply(X = 1:10,
                               FUN = function(x){
                                 ...
                                 sapply(X = seq(0,0.8,0.2),
                                        FUN = function(y){
                                          if (y == 0.8) {
                                            mean((Plot.DF$Starting.PSI[ Plot.DF$length_group == x & Plot.DF$Tissue == each_tissue_name] <= y+0.2) & (Plot.DF$Starting.PSI[ Plot.DF$length_group == x  & Plot.DF$Tissue == each_tissue_name] >= y) , na.rm = T)
                                          } else {
                                            mean((Plot.DF$Starting.PSI[ Plot.DF$length_group == x & Plot.DF$Tissue == each_tissue_name] < y+0.2) & (Plot.DF$Starting.PSI[ Plot.DF$length_group == x  & Plot.DF$Tissue == each_tissue_name] >= y) , na.rm = T)
                                          }
                                        })
...
plots_all_tissues <- do.call(what = "grid.arrange",
                             args = c(plots_per_tissue,
                                      nrow = 6,
                                      ncol = 5,
                                      left = "proportion of exons in each PSI bin",
                                      bottom = "exon length",
                                      top = "exon inclusion vs exon length (27 tissues)"))



ggsave(filename = "117X_all_tissues__gtex_psi_vs_exon_length.pdf",
       width = 2*8.27,
       height = 2*11.69,
       useDingbats = F,
