...
list_of_plot_lists <- vector(mode = "list", length = length(possible_kmers))
names(list_of_plot_lists) <- possible_kmers

for (this_nucleotide in possible_kmers) {
  print(this_nucleotide)
  plots_per_tissue <- vector(mode = "list", length = 0)
  
  for (each_tissue_name in colnames(psi_matrix)) {
    ...
    proportions_matrix <- sapply(X = seq(0,0.5,0.1), # % of kmer
                                 FUN = function(x){
                                   ...
                                   sapply(X = seq(0,0.8,0.2), # PSI
                                          FUN = function(y){
                                            if (y == 0.8) {
                                              mean((psi_matrix[idx,each_tissue_name] <= y+0.2) & (psi_matrix[idx,each_tissue_name] >= y) , na.rm = T)
                                            } else {
                                              mean((psi_matrix[idx,each_tissue_name] < y+0.2) & (psi_matrix[idx,each_tissue_name] >= y) , na.rm = T)
                                            }
                                          })
    ...
    this_plot <- ggplot(data = plot_tibble,
      ...
      scale_x_continuous(breaks = 1:6,
                         labels=c("0–9",
                                  ...
                                  "40–49",
                                  "50+")) +
      ggtitle(paste(unlist(strsplit(each_tissue_name, "(?<=[a-z])(?=[A-Z])",
                                    perl = TRUE)),
                    sep = " ",
                    collapse = " ")) +
      ylab("")
    
    plots_per_tissue[[length(plots_per_tissue)+1]] <- this_plot
    
    
  }
  
  list_of_plot_lists[[this_nucleotide]] <- plots_per_tissue
  

  if (this_nucleotide == "T") {
    this_nucleotide <- "U"
  }
  
  main_title <- paste(this_nucleotide,
                      " content in cassette exons (27 tissues)",
                      sep = "",
                      collapse = "")
  
  plots_all_tissues <- do.call(what = "grid.arrange",
                               args = c(plots_per_tissue,
                                        nrow = 6,
                                        ncol = 5,
                                        left = "percent of exons in each PSI bin",
                                        bottom = "nucleotide content (percent)",
                                        top = ""))
  
  file_name <- paste("121X_",
                     this_nucleotide,
                     "_content_all_tissues.pdf",
                     sep = "",
                     collapse = "")
