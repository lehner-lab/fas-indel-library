...
list_of_plots <- vector(mode = "list",
                        length = 0)


for (each_tissue_name in colnames(psi_matrix)) {
  
  print(each_tissue_name)
  ...
  median_psi_per_kmer <- sapply(X = possible_kmers,
                                FUN = function(x){
                                  ...
                                  median(psi_matrix[idx,each_tissue_name], na.rm = T)
                                })
  ...
  this_plot <- ggplot(data = scatterplot_tibble,
  ...
  list_of_plots[[length(list_of_plots)+1]] <- this_plot
...
plots_all_tissues <- do.call(what = "grid.arrange",
                            args = c(list_of_plots,
                                     nrow = 6,
                                     ncol = 5,
                                     left = "normalised rank (mutation library)",
                                     bottom = "normalised rank (GTEx)",
                                     top = ""))

ggsave(filename = "123X_kmer_rank_all_tissues.pdf",
       width = 2*8.27,
       height = 2*11.69,
       useDingbats = F,
