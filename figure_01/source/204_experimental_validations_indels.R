...
# 
experimental_validations <- readxl::read_excel(path = "204_Validation_Indel_clones_NoBarcode_070324.xlsx", n_max = 17)
...
indels_tibble <- tibble(library_id = sapply(X = experimental_validations$sequence,
                        ...
                        experimental_psi = as.numeric(experimental_validations$`PSI NoBarcode`),
                        experimental_psi_max = sapply(X = experimental_validations$SampleID,
                                                      FUN = function(x){
                                                        idx <- which(experimental_validations$SampleID == x)
                                                        
                                                        psi <- as.numeric(experimental_validations$`PSI NoBarcode`)[idx] + 1.96*(as.numeric(experimental_validations$`SD NoBarcode`)[idx])
...
ggsave(filename = "204_experimental_validation.pdf",
       width = 7,
       height = 7,
       useDingbats = F)
